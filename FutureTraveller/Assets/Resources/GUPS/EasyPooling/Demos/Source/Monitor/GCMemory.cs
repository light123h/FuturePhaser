...
{
    /// <summary>
    /// Monitors and displays Garbage Collection (GC) statistics in real-time, including mean, minimum, and maximum values.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The <see cref="GCMemory"/> class calculates and displays the mean, minimum, and maximum GC memory usage values over a specified history size.
    /// It updates the displayed values in fixed intervals.
    /// </para>
    /// </remarks>
    public class GCMemory : MonoBehaviour
    {
        /// <summary>
        /// The text to display the mean garbage collection.
        /// </summary>
        public UnityEngine.UI.Text MeanText;

        /// <summary>
        /// The text to display the min garbage collection.
        /// </summary>
        public UnityEngine.UI.Text MinText;

        /// <summary>
        /// The text to display the max garbage collection.
        /// </summary>
        ...
        private void Update()
        {
            // Add the current garbage collection value to the list.
            this.AddValue(System.GC.GetTotalMemory(false) / 1024f / 1024f);
        }
        ...
        private void FixedUpdate()
        {
            ...
            {
                this.MeanText.text = string.Format("GC: {0:0.0} MB", this.valueMean);
            }

            // Update the min text.
            if (this.MinText != null)
            {
                this.MinText.text = string.Format("Min: {0:0.0} MB", this.valueMin);
            }

            // Update the max text.
            if (this.MaxText != null)
            {
                this.MaxText.text = string.Format("Max: {0:0.0} MB", this.valueMax);
            }
        }
    }
}
